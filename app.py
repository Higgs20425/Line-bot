...
def responese(msg):
    ...
    '呆寶': '電腦砸了 遊戲刪了', '偉航': ['辣個賺十萬的男人', '再牽一台啦', '三鐵報好玩的啦'], '胖安': ['這很林老闆', '退休了啦', '幫買幾隻狗狗?', '他不會來啦'], '開命': ['有400萬的藍人', '在接客', '很會含', '蝦皮主管啦'], '阿里': '阿里巴巴', '小馬雲': '阿里 阿里巴巴', '蒼哥': '蒼哥在默默操盤啦', '倉哥': '蒼哥在默默操盤啦', 
    '吉哥': ['吉丸吉丸', '大夜在補眠', '真香?'], '雞哥': ['吉丸吉丸', '大夜在補眠', '真香?'], '機哥': ['吉丸吉丸', '大夜在補眠', '真香?'], '基哥': ['吉丸吉丸', '大夜在補眠', '真香?']}
    # '媽媽桑': ['要來我家看貓嗎?', '渣男?'],'靠北': ['嗯?', '嘿?'], '告北': ['嗯?', '嘿?'], '不是阿': ['嗯?', '嘿?'], '很快': '有小業快?', '很快餒': '有小業快?', '快喔': '諭哥準備交割?', 
    # '諭哥哩': '在台東啦', '乾': '嘿?', '不對喔': '出事了阿北', '幹': '又有了?', '....': '....', '問題': '你問題最多'}
    # '': '', 
    # '胞弟': ['雙飛雙飛!', '要驗喔', '有房有老婆 有小孩?', '又在說胞弟壞話?', '壓進去喔', '我就是要雷你'], '老闆': ['石頭開大囉', '把你抱起來X', '就是這麼簡單'], '撈半': ['石頭開大囉', '把你抱起來X', 'Hulk Smash!!!', '就是這麼簡單'], '撈伴': ['石頭開大囉', '把你抱起來X', 'Hulk Smash!!!', '就是這麼簡單'], 
    included_keys = {'有啥好吃': ['源坐?', '葛利麵吃爆?', '背包客?', '鴨肉李伺候?', '米干?', '下次吃茶六?', '碳佐再來?', '伊莉會館海鮮?'],  '真的來了': ['當天再約?', '真的來了餒'], 
    '有啥好吃': ['源坐?', '葛利麵吃爆?', '背包客?', '鴨肉李伺候?', '米干?', '下次吃茶六?', '碳佐再來?', '伊莉會館海鮮?'], '意義': '有意義沒逸逸', '沒聲音': '人走茶涼啦', '會癢': '要驗喔', 
    '會癢?': '要驗餒', '不好說': '不想說都不要說', '不好說啦': '不想說都不要說', '笑死': ['死了沒?', '不要真的笑死餒']
